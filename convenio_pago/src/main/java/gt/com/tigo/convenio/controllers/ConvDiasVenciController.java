...
public class ConvDiasVenciController {
	...
	public @ResponseBody List<ConvDiasVenciDto> listar() {
		...
		for (ConvDiasVenci convenio : lista) {
			ConvDiasVenciDto dto = new ConvDiasVenciDto(convenio.getId(), convenio.getDescripcion(),
					convenio.getValor());
			response.add(dto);
		}
	...
	public ConvDiasVenciDto editar(@PathVariable Long id, @RequestBody ConvDiasVenciDto convDiasVenci) {		
		ConvDiasVenciDto dto = convDiasVenciService.editar(convDiasVenci);
